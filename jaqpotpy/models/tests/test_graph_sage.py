...
from jaqpotpy.models.torch_geometric_models.graph_sage_network import GraphSAGENetwork, GraphSAGEBlock
...
class TestGraphSAGENetwork(unittest.TestCase):
    ...
    def test_no_dropout(self):
        ...
        model = GraphSAGENetwork(self.input_dim, self.hidden_dims, self.output_dim, dropout=0.0)
        data = Data(x=self.x, edge_index=self.edge_index, batch=self.batch)
        output = model(data.x, data.edge_index, data.batch)
        self.assertEqual(output.shape, (self.batch_size, self.output_dim))
    ...
    def test_with_graph_norm(self):
        ...
        model = GraphSAGENetwork(self.input_dim, self.hidden_dims, self.output_dim, graph_norm=True)
        data = Data(x=self.x, edge_index=self.edge_index, batch=self.batch)
        output = model(data.x, data.edge_index, data.batch)
        self.assertEqual(output.shape, (self.batch_size, self.output_dim))
    ...
    def test_different_activation(self):
        ...
        model = GraphSAGENetwork(self.input_dim, self.hidden_dims, self.output_dim, activation=nn.LeakyReLU())
        data = Data(x=self.x, edge_index=self.edge_index, batch=self.batch)
        output = model(data.x, data.edge_index, data.batch)
        self.assertEqual(output.shape, (self.batch_size, self.output_dim))
    ...
    def test_pooling_function_add(self):
        ...
        model = GraphSAGENetwork(self.input_dim, self.hidden_dims, self.output_dim, pooling='add')
        data = Data(x=self.x, edge_index=self.edge_index, batch=self.batch)
    ...
    def test_pooling_function_max(self):
        ...
        model = GraphSAGENetwork(self.input_dim, self.hidden_dims, self.output_dim, pooling='max')
        data = Data(x=self.x, edge_index=self.edge_index, batch=self.batch)
        pooled_output = model._pooling_function(data.x, data.batch)
        self.assertEqual(pooled_output.shape[0], self.batch_size)
