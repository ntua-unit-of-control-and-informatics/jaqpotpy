...
from jaqpotpy.models.torch_geometric_models.graph_attention_network import (
...
class TestGraphAttentionNetwork(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        model = GraphAttentionNetwork(
            self.input_dim, self.hidden_dims, self.heads, self.output_dim
        )
    ...
    def test_forward_pass_node_and_edge_features(self):
        ...
        model = GraphAttentionNetwork(
            self.input_dim, self.hidden_dims, self.heads, self.edge_dim, self.output_dim
        )
        data = Data(
            x=self.x,
    ...
    def test_forward_pass_multiple_instances(self):
        ...
        model_with_edges = GraphAttentionNetwork(
            self.input_dim, self.hidden_dims, self.heads, self.edge_dim, self.output_dim
        )
    ...
    def test_no_dropout(self):
        ...
        model = GraphAttentionNetwork(
            self.input_dim, self.hidden_dims, self.heads, self.output_dim, dropout=0.0
        )
        data = Data(x=self.x, edge_index=self.edge_index, batch=self.batch)
        output = model(data.x, data.edge_index, data.batch)
        self.assertEqual(output.shape, (self.batch_size, self.output_dim))
    ...
    def test_pooling_function_add(self):
        ...
        model = GraphAttentionNetwork(
            self.input_dim, self.hidden_dims, self.heads, self.output_dim, pooling="add"
        )
        data = Data(x=self.x, edge_index=self.edge_index, batch=self.batch)
    ...
    def test_graph_attention_block(self):
        ...
        self.assertEqual(
            output.shape, (self.num_nodes, self.hidden_dims[0] * self.heads)
        )
