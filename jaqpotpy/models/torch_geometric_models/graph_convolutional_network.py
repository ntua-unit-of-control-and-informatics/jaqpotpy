...
class GraphConvolutionNetwork(nn.Module):
    ...
    def __init__(
        ...
        if self.jittable:
            self.graph_layers.append(GCNConv(input_dim, hidden_dim).jittable())
        else:
            self.graph_layers.append(GCNConv(input_dim, hidden_dim))
        for _ in range(hidden_layers):
            if self.jittable:
                self.graph_layers.append(GCNConv(hidden_dim, hidden_dim).jittable())
            else:
                self.graph_layers.append(GCNConv(hidden_dim, hidden_dim))
