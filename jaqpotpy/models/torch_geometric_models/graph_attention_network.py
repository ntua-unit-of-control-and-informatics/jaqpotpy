...
from jaqpotpy.models.torch_geometric_models.fully_connected_network import (
...
class GraphAttentionNetwork(nn.Module):
        ...
        **kwargs,
        ...
        if isinstance(heads, Iterable):
            for item in heads:
                ...
                if item <= 0:
                    raise ValueError(
                        "Each element in the heads list must be between greater than 0"
                    )
        ...
        graph_layer = GraphAttentionBlock(
            input_dim,
            hidden_dims[0],
            heads=self.heads[0],
            edge_dim=edge_dim,
    ...
    def _forward_graph(
        ...
        Returns:
        -------
            Tensor: Output tensor after passing through all graph modules and aggregation function.
