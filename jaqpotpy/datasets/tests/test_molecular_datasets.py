...
class TestDatasets(unittest.TestCase):
    ...
    def test_select_features_with_selection_list(self):
        ...
        self.dataset.select_features(SelectionList=selection_list)

    #     # Verify that only the selected columns remain
    #     self.assertEqual(list(self.dataset.X.columns), selection_list, "Only X1 and X3 should be selected")
    #     self.assertNotIn('X2', self.dataset.X.columns, "X2 should not be in the selected features")
    ...
    #             FeatureSelector=VarianceThreshold(threshold=0.01),
    #             SelectionList=['X1', 'X3']
    #         )
