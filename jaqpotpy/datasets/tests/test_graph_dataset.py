...
class TestSmilesGraphDataset(unittest.TestCase):
    ...
    def test_precompute_featurization(self):
        ...
        dataset = SmilesGraphDataset(smiles=self.smiles, y=self.y, featurizer=self.featurizer)
    ...
    def test_featurization_consistency(self):
        ...
        dataset1 = SmilesGraphDataset(smiles=self.smiles, y=self.y, featurizer=self.featurizer)
        dataset2 = SmilesGraphDataset(smiles=self.smiles, y=self.y, featurizer=self.featurizer)
    ...
    def main_functionality_with_user_inputs(self):
        ...
        for i, data in enumerate(dataset):
            ...
            self.assertEqual(data.x.shape, (num_atoms, len(self.featurizer.get_atom_feature_labels())))
            # Adjacency matrix
            self.assertEqual(data.edge_index.shape[1], num_bonds * 2)  # each bond is represented twice in edge_index
            # Edge features matrix
            self.assertEqual(data.edge_attr.shape, (num_bonds * 2, len(self.featurizer.get_bond_feature_labels())))
            # Check that the features are not empty
            self.assertTrue(torch.is_tensor(data.x) and data.x.numel() > 0)
            self.assertTrue(torch.is_tensor(data.edge_index) and data.edge_index.numel() > 0)
            self.assertTrue(torch.is_tensor(data.edge_attr) and data.edge_attr.numel() > 0)
