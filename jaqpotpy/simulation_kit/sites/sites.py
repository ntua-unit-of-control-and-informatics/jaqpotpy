...
# import numpy as np
#
# from jaqpotpy.simulation_kit import Composition, Element, BravaisLattice
# from jaqpotpy.simulation_kit.utils.utils import pbc_diff
...
#     def __init__(
#         self,
#         species: Union[str, Element, dict, Composition],
#         coords: Union[Sequence[float], Sequence[Sequence[float]], Sequence[np.ndarray], np.ndarray],
...
#
#     @species.setter
#     def species(self, species: Union[Union[str, Element, dict, Composition], Union[str, Element, dict, Composition]]):
#         if not isinstance(species, Composition):
#             try:
#                 species = Composition({species: 1})
#             except TypeError:
...
#         """
#         if self.is_ordered:
#             return list(self.species.as_dict().keys())[0].__str__()
#         sorted_species = sorted(self.species.as_dict().keys())
#         return ", ".join([f"{sp}:{self.species[sp]:.3f}" for sp in sorted_species])
#
#     @property
#     def specie(self) -> Union[Element, str]:
#         """
...
#         return list(self.species.as_dict().keys())[0]
#
#     @property
...
#         """
#         return sum(el.Z for el in self.species.as_dict().keys())
#
...
#         """
#         species_list = []
#         for spec, occu in self.species.as_dict().items():
#             d = {}
#             # del d["@module"]
#             # del d["@class"]
#             d["occu"] = occu
...
#     PeriodicSite includes a BravaisLattice system.
#     """
#
#     def __init__(
#         self,
...
#                 disordered structures.
#         :param coords: Cartesian coordinates of site.
#         :param BravaisLattice: BravaisLattice associated with the site.
...
#             frac_coords = BravaisLattice.get_fractional_coords(coords)
#         else:
...
#         self._frac_coords: Union[Sequence[float], Sequence[Sequence[float]], Sequence[np.ndarray], np.ndarray] = frac_coords
#         self._species: Composition = species  # type: ignore
#         self._coords: Optional[np.ndarray] = None
#         self.properties: dict = properties or {}
#
#         super().__init__(species, coords, properties, skip_checks)
#
#     def __hash__(self):
#         """
#         Minimally effective hash function that just distinguishes between Sites
...
#         """
#         BravaisLattice associated with PeriodicSite
#         """
#         return self._lattice
#
#     @lattice.setter
#     def lattice(self, BravaisLattice: BravaisLattice):
#         """
#         Sets BravaisLattice associated with PeriodicSite
#         """
#         self._lattice = BravaisLattice
...
#             return None
#         return PeriodicSite(self.species, frac_coords, self.BravaisLattice, properties=self.properties)
#
...
#         """
#         if check_lattice and self.BravaisLattice != other.BravaisLattice:
#             return False
...
#         return (
#             self.species == other.species
#             and self.BravaisLattice == other.BravaisLattice
#             and np.allclose(self.coords, other.coords, atol=Site.position_atol)
#             and self.properties == other.properties
#         )
#
...
#         self, fcoords: Union[Sequence[float], Sequence[Sequence[float]], Sequence[np.ndarray], np.ndarray] , jimage: Optional[Union[Sequence[float], Sequence[Sequence[float]], Sequence[np.ndarray], np.ndarray] ] = None
...
#         returns the distance and jimage indices in terms of BravaisLattice vector
...
#                 BravaisLattice translations, e.g., [1,0,0] implies to take periodic
#                 image that is one a-BravaisLattice vector away. If jimage is None,
#                 the image that is nearest to the site is found.
#         Returns:
#             (distance, jimage): distance and periodic BravaisLattice translations
#             of the other site for which the distance applies.
#         """
#         return self.BravaisLattice.get_distance_and_image(self.frac_coords, fcoords, jimage=jimage)
#
#     def distance_and_image(self, other: "PeriodicSite", jimage: Optional[Union[Sequence[float], Sequence[Sequence[float]], Sequence[np.ndarray], np.ndarray] ] = None) -> Tuple[float, np.ndarray]:
#         """
...
#             jimage (3x1 array): Specific periodic image in terms of BravaisLattice
#                 translations, e.g., [1,0,0] implies to take periodic image
#                 that is one a-BravaisLattice vector away. If jimage is None,
#                 the image that is nearest to the site is found.
#         Returns:
#             (distance, jimage): distance and periodic BravaisLattice translations
#             of the other site for which the distance applies.
...
#
#     def distance(self, other: "PeriodicSite", jimage: Optional[Union[Sequence[float], Sequence[Sequence[float]], Sequence[np.ndarray], np.ndarray] ] = None):
#         """
...
#         """
#         species_list = []
#         for spec, occu in self._species.as_dict().items():
#             d = {}
#             d["occu"] = occu
#             d["element"] = spec
#             species_list.append(d)
...
#             lattice: Optional BravaisLattice to override BravaisLattice specified in d.
...
#             for key in props.keys():
#                 props[key] = json.loads(json.dumps(props[key]))
#         lattice = lattice if lattice else d["lattice"]
