...
def pdbqt_to_pdb(
    filename: Optional[str] = None, pdbqt_data: Optional[List[str]] = None
) -> str:
...
def _create_pdb_map(outfile: str) -> Dict[int, str]:
    ...
    for line in lines:
        ...
        atom_symbol = atom_symbol.replace("MG", "Mg")
        atom_symbol = atom_symbol.replace("MN", "Mn")
        atom_symbol = atom_symbol.replace("ZN", "Zn")
        atom_symbol = atom_symbol.replace("CA", "Ca")
        atom_symbol = atom_symbol.replace("FE", "Fe")
        atom_symbol = atom_symbol.replace("CL", "Cl")
        atom_symbol = atom_symbol.replace("SI", "Si")
        atom_symbol = atom_symbol.replace("BR", "Br")
        atom_symbol = atom_symbol.replace("CU", "Cu")
        atom_symbol = atom_symbol.replace("CA", "Ca")
        atom_symbol = atom_symbol.replace("AT", "At")
...
def _create_component_map(mol: RDKitMol, components: List[List[int]]) -> Dict[int, int]:
...
def _dfs(
    ...
    for b in rotatable_bonds:
        valid, next_partition = _valid_bond(
            used_partitions, b, current_partition, comp_map
        )
        if not valid:
            continue
        lines = _dfs(
