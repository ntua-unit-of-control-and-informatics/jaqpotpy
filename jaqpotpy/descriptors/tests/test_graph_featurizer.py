...
class TestGraphFeaturizer(unittest.TestCase):
    ...
    def test_supported_bond_features(self):
        ...
        expected_bond_features = ["bond_type", "is_conjugated", "is_in_ring", "stereo"]
    ...
    def test_default_featurizer(self):
        ...
        atom_features, bond_features = self.featurizer.extract_molecular_features(
            self.mol
        )
        # Dtype assertion
    ...
    def test_user_creating_node_features(self):
        ...
        self.featurizer.add_atom_feature(
            "symbol",
        ...
        # Number of nodes
        self.assertEqual(
            data.x.shape[0], self.mol.GetNumAtoms(), "Number of nodes is incorrect"
        )
        # Number of node features
        self.assertEqual(
            data.x.shape[1], node_dims, "Dimension of node feature vector is incorrect"
        )
        # Dtype assertion
    ...
    def test_user_creating_bond_features(self):
        ...
        self.assertEqual(
            ...
            edge_dims,
            "Dimension of edge feature vector is incorrect",
        )
        # Dtype assertion
    ...
    def test_correct_adjacency_matrix_dimensions(self):
        ...
        data = self.featurizer.featurize(self.smile)
        self.assertEqual(
            data.edge_index.shape[0], 2, "Needs to be 2 in order to match COO format"
        )
        self.assertEqual(
            data.edge_index.shape[1],
            self.mol.GetNumBonds() * 2,
            "Number of edges is incorrect",
        )
