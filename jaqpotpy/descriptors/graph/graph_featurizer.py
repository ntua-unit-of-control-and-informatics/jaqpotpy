...
class SmilesGraphFeaturizer():
    ...
    def add_bond_feature(self, bond_feature, allowable_set=None):
        ...
        This allows handling of unexpected values by mapping them to a default or placeholder category.
        """
        # The name of the bond feature must be exactly as given by RDKit
        ...
        if bond_feature in self.NON_ONE_HOT_ENCODED:
            ...
            if not isinstance(allowable_set, (list, tuple)):
                raise TypeError("Input dictionary must have values of type list, tuple or None.")
            # Creates the feature list that will be transformed in one-hot
            self.bond_allowable_sets[bond_feature] = list(allowable_set)
    
    def get_atom_feature_labels(self):
        """Returns a list of labels for the atom features"""
        atom_feature_labels = []
    ...
    def get_bond_feature_labels(self):
        """Returns a list of labels for the bond features"""
        bond_feature_labels = []
        ...
        for feature in self.atom_allowable_sets.keys():
            ...
            if feature in self.NON_ONE_HOT_ENCODED:
                ...
                if 'UNK' in allowable_set:
                    one_hot_encoded_feat = self._one_of_k_encoding_unk(feat, allowable_set)
                else:
                    one_hot_encoded_feat = self._one_of_k_encoding(feat, allowable_set, feature_name=feature)
