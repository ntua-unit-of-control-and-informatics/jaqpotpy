...
class SmilesToImage(MolecularFeaturizer):
    ...
    >>> from jaqpotpy.descriptors import SmilesToImage
    ...
    def _featurize(self, datapoint: RDKitMol, **kwargs) -> np.ndarray:
        ...
        if self.img_spec == "std":
            ...
            bond_props = np.array(
                [
                    [2.0, bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()]
        ...
        bond_line_idxs = np.ceil((line_coords[:, 0] + self.embed) / self.res).astype(
            int
        )
        bond_line_idys = np.ceil((line_coords[:, 1] + self.embed) / self.res).astype(
            int
        )
        # Turn atomic coordinates into image positions
        atom_idxs = np.round((atom_coords[:, 0] + self.embed) / self.res).astype(int)
        atom_idys = np.round((atom_coords[:, 1] + self.embed) / self.res).astype(int)
