...
class PagtnMolGraphFeaturizer(MolecularFeaturizer):
    ...
    def _pagtn_atom_featurizer(self, atom: RDKitAtom) -> np.ndarray:
        ...
        formal_charge = get_atom_formal_charge_one_hot(atom, include_unknown_set=False)
    ...
    def _edge_features(
        self, mol: RDKitMol, path_atoms: Tuple[int, ...], ring_info
    ) -> np.ndarray:
        ...
        for path_idx in range(path_length - 1):
            bond = mol.GetBondBetweenAtoms(
                path_atoms[path_idx], path_atoms[path_idx + 1]
            )
            if bond is None:
                import warnings
        ...
        if ring_info:
            ...
            features.append(
                [0.0] + one_hot_encode(ring_info, allowable_set=self.RING_TYPES)
            )
        return np.concatenate(features, axis=0)

    def _pagtn_edge_featurizer(self, mol: RDKitMol) -> Tuple[np.ndarray, np.ndarray]:
...
class AttentiveFPFeaturizer(MolecularFeaturizer):
    ...
    def _featurize(self, datapoint: RDKitMol, **kwargs) -> GraphData:
        ...
        # construct atom (node) feature
        node_features = np.asarray(
            [self._featurize_node(atom) for atom in datapoint.GetAtoms()], dtype=float
        )
        ...
        bond_features = np.asarray(
            [self._featurize_edge(bond, datapoint) for bond in datapoint.GetBonds()],
            dtype=float,
        )  # deafult None
        bond_features = bond_features.reshape(
            bond_features.shape[0] * bond_features.shape[1], bond_features.shape[2]
        )
