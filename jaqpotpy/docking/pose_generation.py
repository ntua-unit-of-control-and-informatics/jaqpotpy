...
The code is taken from the deep chem project
https://github.com/deepchem/
"""
...
from jaqpotpy.docking.binding_pocket import BindingPocketFinder
from jaqpotpy.utils.data_utils import download_url, get_data_dir
from jaqpotpy.utils.types import RDKitMol
from jaqpotpy.utils.geometry_utils import compute_centroid, compute_protein_range
...
class GninaPoseGenerator(PoseGenerator):
    ...
    * Almost all dependencies are included in the most compatible way
    ...
    def generate_poses(
        ...
        **kwargs,
    ) -> Union[Tuple[DOCKED_POSES, np.ndarray], DOCKED_POSES]:
    ...
    def generate_poses(
        ...
        **kwargs,
        ...
        if centroid is not None and box_dims is not None:
            ...
            if self.pocket_finder is None:
                ...
                for pocket in pockets:
                    ...
                    x_box = (x_max - x_min) / 2.0
                    y_box = (y_max - y_min) / 2.0
                    z_box = (z_max - z_min) / 2.0
        ...
        for i, (protein_centroid, box_dims) in enumerate(zip(centroids, dimensions)):
            ...
            vpg.write_poses(
                out_pdbqt, n_poses=num_modes, energy_range=energy_range, overwrite=True
            )
